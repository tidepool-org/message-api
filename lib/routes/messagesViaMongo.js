...
module.exports = function(config) {
    ...
    */
    ...
    var findById = function(req, res, next) {
        log.info("Request came in!  params[%j], url[%s], method[%s]", req.params, req.url, req.method);
        ...
        if(objectIdIsValid(messageId)){
            ...
            messagesCollection.findOne({
                ...
                if (err) {
                    var errMessage = util.format('Problem finding message params[%j].', req.params);
    ...
    var findAllById = function(req, res, next) {
        ...
        messagesCollection.find({ 
            ...
            if (err) {
                var errMessage = util.format('Problem finding messages params[%j].', req.params)
                log.error(err,errMessage);
                res.send({'error':err});
            } else {
                if(doc && doc.length>0){
    ...
    var add = function(req, res, next) {
        ...
        if(messageToSaveIsValid(message)){
            ...
            messagesCollection.save(message, function(err, doc) {
                if (err) {
                    var errMessage = util.format('Problem adding message[%j].', message);
                    log.error(err,errMessage);
                    res.send({'error':err});
                } else {
