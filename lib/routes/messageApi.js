...
module.exports = function(config, crudHandler, seagullHandler, metrics) {
  ...
  /*
   * Format the message that we will save
   */
  ...
  /*
   * Format a reply/comment on a message that we will save
   */
  ...
  /*
   * Check that the object has the given list of properties
   */
  ...
  function getDeletionWindow(){
    return moment().utc().add('days', deleteWindowTime)
  }
  ...
  return {
     ...
     *
     * @param {Object} req
     * @param {Object} req.params.msgid id of the message we are editing
    ...
    editMessage : function(req, res, next) {
      ...
      var update = req.params.message;
      var messageId = req.params.msgid;

      if(_.isEmpty(update.id) && _.isEmpty(messageId)){
        res.send(400,'msgid is not defined');
        return next();
      }
      //set the id
      update.id = update.id || messageId;

      crudHandler.editMessage(update,function(error, details){

        if(error){
          log.error(error, 'Error editing message[%j]', update);
          res.send(500);
        } else {
          console.log('edited message ',details);
          res.send(200, { message: details });
        }
        return next();
      });
    },
    ...
    removeMessage : function(req, res, next) {
      ...
      crudHandler.deleteMessage(messageId,function(error, details){
        if(error){
          log.error(error, 'Error removing message[%s]', messageId);
          res.send(500);
        } else {
          console.log('removed message ',details);
          res.send(202);
        }
        return next();
      });
