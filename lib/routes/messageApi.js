...
module.exports = function (config, crudHandler, seagullHandler, metrics) {
  ...
  return {
    ...
    findById: function (req, res, next) {
      log.debug(
        'findById: params[%j], url[%s], method[%s]',
        req.params,
        req.url,
        req.method
      );
    ...
    findAllById: function (req, res, next) {
      log.debug(
        'findAllById: params[%j], url[%s], method[%s]',
        req.params,
        req.url,
        req.method
      );
      ...
      try {
        startDate = utility.getISODate(req.query.starttime);
        endDate = utility.getISODate(req.query.endtime);
      } catch (error) {
        log.error(
          error,
          'findAllById error parsing dates start[%s] end[%s]',
          startDate,
          endDate
        );
        res.send(400);
      ...
      crudHandler.getAllMessages(
        ...
        function (error, messages) {
          if (error) {
            log.error(
              error,
              'Error getting messages for group[%s] from[%s] to[%s]',
              groupId,
              startDate,
              endDate
            );
            ...
            log.info(
              'findAllById: messages (n=%d) received in [%s]millis for [%s] ',
              messages.length,
              Date.now() - started,
              groupId
            );
            ...
            resolveUserProfiles(messages, function (resolvedMessages) {
              log.info(
                'findAllById: profiles resolved in [%s]millis for [%s] ',
                Date.now() - started,
                groupId
              );
    ...
    getNotes: function (req, res, next) {
      log.debug(
        'getNotes: params[%j], url[%s], method[%s]',
        req.params,
        req.url,
        req.method
      );
      ...
      try {
        ...
        log.error(
          error,
          'getNotes error parsing dates start[%s] end[%s]',
          startDate,
          endDate
        );
        res.send(400);
      ...
      crudHandler.getNotes(
        ...
        function (error, messages) {
          if (error) {
            ...
            log.info(
              'getNotes: messages (n=%d) received in [%s]millis for [%s] ',
              messages.length,
              Date.now() - started,
              groupId
            );
            ...
            resolveUserProfiles(messages, function (resolvedMessages) {
              log.info(
                'getNotes: profiles resolved in [%s]millis for [%s] ',
                Date.now() - started,
                groupId
              );
              res.send(200, { messages: resolvedMessages });
    ...
    getThread: function (req, res, next) {
      log.debug(
        'getThread: params[%j], url[%s], method[%s]',
        req.params,
        req.url,
        req.method
      );
      ...
      crudHandler.getMessagesInThread(messageId, function (error, messages) {
        if (error) {
          ...
          log.info(
            'getThread: thread (n=%d) received in [%s]millis for [%s] ',
            messages.length,
            Date.now() - started,
            messageId
          );
          ...
          resolveUserProfiles(messages, function (resolvedMessages) {
            log.info(
              'getThread: profiles resolved in [%s]millis for [%s] ',
              Date.now() - started,
              messageId
            );
            res.send(200, { messages: resolvedMessages });
     ...
     *
     * @param {Object} req
     * @param {String} req.params.groupid the group the message is for
    ...
    addThread: function (req, res, next) {
      log.debug(
        'addThread: body[%j], url[%s], method[%s]',
        req.body,
        req.url,
        req.method
      );
      ...
      if (!_.isEmpty(missing)) {
        ...
        var message = format.incomingMessage(
          req._tokendata.userid,
          req.params.groupid,
          req.body.message
        );
    ...
    replyToThread: function (req, res, next) {
      log.debug(
        'replyToThread: params[%j], body[%j], url[%s], method[%s]',
        req.params,
        req.body,
        req.url,
        req.method
      ...
      if (!_.isEmpty(missing)) {
        ...
        var reply = format.incomingReply(
          req._tokendata.userid,
          req._message,
          req.body.message
        );
     ...
     * @param {Object} req.body.message content we are updating
     * @param {String} req.body.message.timestamp the updated ISO_8601 formated string
     * @param {String} req.body.message.messagetext the updated message text
     * @param {Object} res
     * @param {Function} next
     * @return next
     */
    editMessage: function (req, res, next) {
      log.debug(
        'editMessage: params[%j], body[%j], url[%s], method[%s]',
        req.params,
        req.body,
        req.url,
        req.method
    ...
    removeMessage: function (req, res, next) {
      log.debug(
        'removeMessage: params[%j], url[%s], method[%s]',
        req.params,
        req.url,
        req.method
      );
      ...
      crudHandler.deleteMessage(
        req.params.msgid,
        updates,
        function (error, deletionDetails) {
          if (error) {
