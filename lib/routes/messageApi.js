...
module.exports = function(config, crudHandler, seagullHandler, metrics) {
  ...
  function getDeletionWindow(){
    var theDate =  moment().utc().add('days', deleteWindowTime);
  ...
  return {
    ...
    removeMessage : function(req, res, next) {
      ...
      crudHandler.deleteMessage(messageDetails,function(error, deletionDetails){

        if(error){
          log.error(error, 'Error removing message[%s]', messageDetails.id);
          res.send(500);
        } else {
          res.send(202);
        }
