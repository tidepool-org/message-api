...
module.exports = function(gatekeeperClient) {
  ...
  return {
    /**
     * Does this user have permisson to see the requested users messages
     */
    checkPermissionsFromGroup: function(req, res, next) {

      log.debug('checking permisson [%j]', req.params);
      ...
      if(req.params.groupid && req._tokendata.userid){
        ...
        gatekeeperClient.canViewData(presentUser, requestedUser,function(error,canView){
          if (canView) {
            log.debug('user permisson [%s] has permisson to see [%s]',presentUser, requestedUser);
            return next();
          }
          log.warn('no permisson [%s] for requested user [%s]', presentUser, requestedUser);
          res.send(401, 'You are a Balrog and I am Gandalf. EOM');
          return next(false);
        });
      }
      log.error('do not have the required information to check permissons');
      return next(false);
    },
    /**
     * Does this user have permisson to add message
     */
