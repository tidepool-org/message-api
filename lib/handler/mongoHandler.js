...
module.exports = function(connectionString) {
  ...
  */
  ...
  function findComments(messages,cb){
    async.map(messages,function(message,callback){
      if(_.isEmpty(message.parentmessage)){
        messagesCollection.count( { parentmessage : String(message.id) },function(error,count){
  ...
  return {
    ...
    getAllMessages: function(groupId, start, end, callback) {
      ...
      async.waterfall(
        [
          function(cb) {
            getMessagesForOptions(findOptions,cb);
          },
          function(messages, cb) {
            findComments(messages,cb);
          }
        ],
