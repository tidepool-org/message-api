...
module.exports = function(connectionString) {
  ...
  */
  ...
  return {
    ...
    getAllMessages: function(patientId, start, end, callback) {

      log.debug('Finding all messages for patient[%s] from[%s] to[%s]', patientId, start, end);
      ...
      messagesCollection
        .find({ timestamp : timeQueryOptions, patientid : patientId })
        .sort({timestamp:-1},
        function(error,docs){
          if (error) {
            return callback(error,null);
    ...
    getNotes: function(patientId, callback) {

      log.debug('Finding all notes for patient[%s]', patientId);

      // -1 to sort in descending order or a 1 to sort in ascending
      messagesCollection
        .find({ parentmessage : null, patientid : patientId })
        .sort({timestamp:-1},
        function(error,docs){
          if (error) {
            return callback(error,null);
