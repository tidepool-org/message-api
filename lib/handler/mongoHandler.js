...
module.exports = function (connectionString) {
  ...
  let dependencyStatus = { running: false, deps: { up: [], down: [] } };

  let initialConnection = false;
  const dbInstance = mongojs(connectionString, [messagesCollectionName]);
  ...
  dbInstance.on('connect', function () {
    ...
    if(!initialConnection) {
      messagesCollection.createIndex({ groupid: 1, timestamp: 1 });
  ...
  let messagesCollection = dbInstance.collection(messagesCollectionName);
  // Running a ping command in order to trigger connect or error event on startup
  dbInstance.runCommand({ping: 1});
