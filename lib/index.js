...
(async function () {
  ...
  var metrics = require('user-api-client').metrics(metricsSpec, config, log);
  ...
  var crudHandler = require('./handler/mongoHandler')(
    config.mongoDbConnectionString
  );

  const eventsLogger = logMaker('handler/events.js');
  const eventsConfig = amoeba.events.loadConfigFromEnv();
  const userEventsHandler = events.createUserEventsHandler(
    crudHandler,
    eventsLogger
  );
  const consumer = await amoeba.events.createEventConsumer(
    eventsConfig,
    userEventsHandler,
    eventsLogger
  );
