...
var messagesService = function(envConfig, crudHandler, userApiClient, seagullHandler) {
...
function createServer(serverConfig, crudHandler, userApiClient, seagullHandler){
    ...
    var messages = require('./routes/messageApi')(crudHandler,seagullHandler);
    ...
    server.get('/all/:patientid?starttime&endtime', checkToken, messages.findAllById);
    server.get('/thread/:msgid', checkToken, messages.getThread);

    server.get('/notes/:patientid', checkToken, messages.getNotes);

    //adding messages
    server.post('/send/:patientid', checkToken, messages.addThread);
