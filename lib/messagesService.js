...
function createServer(serverConfig, envConfig, crudHandler, userApiHostGetter){
    ...
    server.get('/read/:msgid', userApi.checkToken, userApi.getToken, messages.findById);
    server.get('/all/:groupid?starttime&endtime', userApi.checkToken, userApi.getToken, messages.findAllById);
    
    //adding messages
    server.post('/send/:groupid', userApi.checkToken, userApi.getToken, messages.add);
